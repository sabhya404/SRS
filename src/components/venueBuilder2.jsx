...
export default function VenueBuilder({ eventId, event }) {
  ...
  // Get the actual used dimensions based on assigned seats
  const getUsedDimensions = () => {
    ...
    // If no seats are assigned, return minimum dimensions
      ...
      for (const category of event.categories) {
        ...
        for (const sub of category.subcategories) {
          ...
          if (subCount !== sub.subSeats) {
            alert(
              `Subcategory ${sub.subName} requires exactly ${sub.subSeats} seats (${subCount} assigned)`
            );
            return;
          }
        }
